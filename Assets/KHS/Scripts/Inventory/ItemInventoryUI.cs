    ...
    private void Init()
    {
        ...
        for (int _invenCnt = 0; _invenCnt < _itemCnt; _invenCnt++)
        {
            ...
            if (item._itemPart == Item.ItemPart.GRIMOIRE)
                _itemObj.sprite = Resources.Load<Sprite>("Sprites/obj" + Item.PartToPartID(item._itemPart) + Item.duddjfmfTMwk(item));
            else _itemObj.sprite = Resources.Load<Sprite>("Sprites/obj" + Item.PartToPartID(item._itemPart));
            ///////
            ...
            _itemGradeFrame.sprite = Resources.Load<Sprite>("Sprites/frame" + item._itemGradeID);
